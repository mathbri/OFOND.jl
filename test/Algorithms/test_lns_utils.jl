...
@testset "Arc variable filter" begin
    # has_bundle_arc_var with different arcs
    @test OFOND.has_bundle_arc_var(TTGraph, supp1FromDel2, plantFromDel0, [supp1FromDel2])
    @test !OFOND.has_bundle_arc_var(TTGraph, supp1FromDel2, plantFromDel0, [supp2FromDel1])
    @test OFOND.has_bundle_arc_var(TTGraph, xdockFromDel1, plantFromDel0, [supp1FromDel2])
end
...
@testset "Add variables" begin
    ...
    # println(model[:x])
    @test num_variables(model) == 42
    @test length(model[:x][1, :]) == 10
    @test length(model[:x][2, :]) == 8
    @test length(model[:x][3, :]) == 12
...
@testset "Add path constraints" begin
    ...
    # Init path constraints 
    path_expr = OFOND.init_path_expr(model, TTGraph, [1, 2, 3])
    I, J, V = findnz(path_expr)
    @test I == vcat(
        [1, 3, 1, 3, 1, 3, 3, 2, 2, 1, 2, 3, 1, 2, 3, 1],
        [2, 3, 1, 2, 3, 1, 2, 3, 1, 2, 3, 1, 2, 3, 1, 2, 3],
    )
    @test J == vcat(
        [1, 1, 2, 2, 3, 3, 4, 5, 6, 7, 7, 7, 8, 8, 8, 9, 9, 9, 10, 10, 10],
        [11, 11, 11, 12, 12, 12, 13, 13, 13, 15, 15, 15],
    )
    @test all(V .== AffExpr(0))
    ...
    @test num_constraints(model; count_variable_in_set_constraints=false) == 33
    @test length(model[:path][:, 1]) == 11
    x = model[:x]
    # println(collect(edges(TTGraph.graph)))

    # Creating edge index matrix for ease of testing
    ...
    @test edgeIndex == OFOND.create_edge_index(TTGraph)

    # Node 1 : x[1, (2, 1)] == 0,
    path11 = constraint_object(model[:path][1, 1])
    @test path11.func == AffExpr(0, x[1, edgeIndex[2, 1]] => 1)
    @test path11.set == MOI.EqualTo(0.0)

    # Node 2 : -x[1, (2, 1)] - x[1, (2, 7)] + x[1, (3, 2)] == 0,
    path12 = constraint_object(model[:path][2, 1])
    @test path12.func == AffExpr(
        0,
        x[1, edgeIndex[2, 1]] => -1,
        x[1, edgeIndex[2, 7]] => -1,
        x[1, edgeIndex[3, 2]] => 1,
    ...
    # Node 3 : -x[1, (3, 2)] - x[1, (3, 8)] - x[1, (3, 15)] == -1,
    path13 = constraint_object(model[:path][3, 1])
    @test path13.func == AffExpr(
        0,
        x[1, edgeIndex[3, 2]] => -1,
        x[1, edgeIndex[3, 8]] => -1,
        x[1, edgeIndex[3, 15]] => -1,
    )
    @test path13.set == MOI.EqualTo(-1.0)

    # Node 4, 5, 6, 7 are not concerned by bundle 1

    # Node 7 : x[1, (2, 7)] == 0,
    path17 = constraint_object(model[:path][7, 1])
    @test path17.func == AffExpr(0, x[1, edgeIndex[2, 7]] => 1)
    @test path17.set == MOI.EqualTo(0.0)

    # Node 8 : x[1, (3, 8)] - x[1, (8, 11)] - x[1, (8, 15)] == 0,
    path18 = constraint_object(model[:path][8, 1])
    @test path18.func == AffExpr(
        0,
        x[1, edgeIndex[3, 8]] => 1,
        x[1, edgeIndex[8, 11]] => -1,
        x[1, edgeIndex[8, 15]] => -1,
    )
    @test path18.set == MOI.EqualTo(0.0)

    # Node 9 : - x[1, (9, 12)] == 0,
    path19 = constraint_object(model[:path][9, 1])
    @test path19.func == AffExpr(0, x[1, edgeIndex[9, 12]] => -1)
    @test path19.set == MOI.EqualTo(0.0)

    # Node 10 : -x[1, (10, 13)] == 0,
    path20 = constraint_object(model[:path][10, 1])
    @test path20.func == AffExpr(0, x[1, edgeIndex[10, 13]] => -1)
    @test path20.set == MOI.EqualTo(0.0)

    # Node 11 : -x[1, (8, 11)] == 0,
    path21 = constraint_object(model[:path][11, 1])
    @test path21.func == AffExpr(0, x[1, edgeIndex[8, 11]] => 1)
    @test path21.set == MOI.EqualTo(0.0)

    # Node 12 : x[1, (9, 12)] - x[1, (12, 15)] == 0,
    path22 = constraint_object(model[:path][12, 1])
    @test path22.func ==
        AffExpr(0, x[1, edgeIndex[9, 12]] => 1, x[1, edgeIndex[12, 15]] => -1)
    @test path22.set == MOI.EqualTo(0.0)

    # Node 13 : x[1, (10, 13)] == 0,
    path23 = constraint_object(model[:path][13, 1])
    @test path23.func == AffExpr(0, x[1, edgeIndex[10, 13]] => 1)
    @test path23.set == MOI.EqualTo(0.0)

    # Node 15 : x[1, (3, 15)] + x[1, (8, 15)] + x[1, (12, 15)] == 1,
    path25 = constraint_object(model[:path][15, 1])
    @test path25.func == AffExpr(
        0,
        x[1, edgeIndex[3, 15]] => 1,
        x[1, edgeIndex[8, 15]] => 1,
        x[1, edgeIndex[12, 15]] => 1,
    )
    @test path25.set == MOI.EqualTo(1.0)

    @test length(model[:path][:, 2]) == 10
    @test length(model[:path][:, 3]) == 12
    ...
    OFOND.add_old_new_path_constraints!(
        model, [bundle1, bundle2], oldPaths, newPaths, edgeIndex
    )
    ...
    @test oldPath11.func == AffExpr(0, x[1, edgeIndex[3, 15]] => -1, z[1] => -1)
    ...
    @test oldPath12.func == AffExpr(0, x[2, edgeIndex[6, 15]] => 1)
...
@testset "Packing constraints" begin
    sol = OFOND.Solution(TTGraph, TSGraph, bundles)
    relaxedSol = OFOND.RelaxedSolution(sol, instance, bundles)
    edgeIndex = OFOND.create_edge_index(TTGraph)
    model = Model(HiGHS.Optimizer)
    OFOND.add_variables!(model, :single_plant, instance, relaxedSol)
    ...
    OFOND.complete_packing_expr!(model, expr_mat, instance, [bundle1], edgeIndex)
    @test expr_mat[xdockStep4, plantStep1] == AffExpr(
        25,
        model[:tau][(xdockStep4, plantStep1)] => -50,
        model[:x][1, edgeIndex[xdockFromDel1, plantFromDel0]] => 20,
    )
    @test expr_mat[xdockStep4, portStep1] == AffExpr(
        0,
        model[:tau][(xdockStep4, portStep1)] => -50,
        model[:x][1, edgeIndex[xdockFromDel1, portFromDel0]] => 20,
    )
    @test expr_mat[xdockStep1, plantStep2] ==
        AffExpr(0, model[:tau][(xdockStep1, plantStep2)] => -50)

    # add packing constraints with a dummy model
    OFOND.add_packing_constraints!(model, instance, bundles, sol, edgeIndex)
    ...
    @test packing914.func == AffExpr(0, x[3, edgeIndex[8, 11]] => 25, tau[(9, 14)] => -50)
    # packing[(10, 15)] : 20 x[1, (10, 13)] + 25 x[3, (10, 13)] - 50 tau[(10, 15)] <= 0
    packing1015 = constraint_object(model[:packing][(10, 15)])
    # @test packing1015.func == AffExpr(
    #     0,
    #     x[1, edgeIndex[10, 13]] => 20,
    #     x[3, edgeIndex[10, 13]] => 25,
    #     tau[(10, 15)] => -50,
    # )
    @test packing1015.func ==
        AffExpr(0, x[3, edgeIndex[10, 13]] => 25, tau[(10, 15)] => -50)
    ...
    @test packing1116.func == AffExpr(
        0,
        x[1, edgeIndex[9, 12]] => 20,
        x[3, edgeIndex[9, 12]] => 25,
        x[3, edgeIndex[10, 13]] => 25,
        tau[(11, 16)] => -50,
    )
    ...
    @test packing1213.func == AffExpr(
        0,
        x[1, edgeIndex[8, 11]] => 20,
        x[2, edgeIndex[8, 11]] => 30,
        x[3, edgeIndex[8, 11]] => 25,
        x[3, edgeIndex[9, 12]] => 25,
        tau[(12, 13)] => -50,
    )
    # packing[(9, 18)] : 25 x[3, (8, 15)] - 50 tau[(9, 18)] <= 0
    packing918 = constraint_object(model[:packing][(9, 18)])
    @test packing918.func == AffExpr(0, x[3, edgeIndex[8, 15]] => 25, tau[(9, 18)] => -50)
    ...
    @test packing1217.func == AffExpr(
        0,
        x[1, edgeIndex[8, 15]] => 20,
        x[2, edgeIndex[8, 15]] => 30,
        x[3, edgeIndex[8, 15]] => 25,
        tau[(12, 17)] => -50,
    )
    # packing[(13, 18)] : 25 x[3, (12, 15)] - 50 tau[(13, 18)] <= 0
    packing1318 = constraint_object(model[:packing][(13, 18)])
    @test packing1318.func ==
        AffExpr(0, x[3, edgeIndex[12, 15]] => 25, tau[(13, 18)] => -50)
    ...
    @test packing1617.func == AffExpr(
        0,
        x[1, edgeIndex[12, 15]] => 20,
        x[2, edgeIndex[12, 15]] => 30,
        x[3, edgeIndex[12, 15]] => 25,
        tau[(16, 17)] => -50,
    )
end
...
@testset "Adding all constraints" begin
    ...
    @test num_constraints(model; count_variable_in_set_constraints=false) == 41
    @test length(model[:packing]) == 12
    @test length(model[:path][:, 1]) == 11
    @test length(model[:path][:, 3]) == 12
...
@testset "Add objective" begin
    ...
    edgeIndex = OFOND.create_edge_index(TTGraph)
    add_to_expression!(objExprTest, x[1, edgeIndex[supp1FromDel1, supp1FromDel0]], 1e-5)
    add_to_expression!(
        objExprTest, x[1, edgeIndex[supp1FromDel1, xdockFromDel0]], 5.004003999999999
    )
    add_to_expression!(objExprTest, x[1, edgeIndex[supp1FromDel2, supp1FromDel1]], 1e-5)
    add_to_expression!(
        objExprTest, x[1, edgeIndex[supp1FromDel2, xdockFromDel1]], 5.004003999999999
    )
    add_to_expression!(objExprTest, x[1, edgeIndex[supp1FromDel2, plantFromDel0]], 10.00401)
    add_to_expression!(objExprTest, x[1, edgeIndex[xdockFromDel1, portFromDel0]], 5.0)
    add_to_expression!(objExprTest, x[1, edgeIndex[xdockFromDel1, plantFromDel0]], 5.004)
    add_to_expression!(objExprTest, x[1, edgeIndex[xdockFromDel2, portFromDel1]], 5.0)
    add_to_expression!(objExprTest, x[1, edgeIndex[xdockFromDel3, portFromDel2]], 5.0)
    add_to_expression!(objExprTest, x[1, edgeIndex[portFromDel1, plantFromDel0]], 5.004)
...
@testset "Warm start" begin
    ...
    portFromDel1 = TTGraph.hashToIdx[hash(1, port_l.hash)]
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, plantFromDel0]])
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, xdockFromDel1]])
    ...
    OFOND.warm_start_milp!(model, :single_plant, instance, relaxedSol, edgeIndex)
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, plantFromDel0]])
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, xdockFromDel1]])
    ...
    relaxedSol = OFOND.RelaxedSolution(sol, instance, [bundle1])
    OFOND.warm_start_milp!(model, :single_plant, instance, relaxedSol, edgeIndex)
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, supp1FromDel1]])
    @test start_value(model[:x][1, edgeIndex[supp1FromDel2, plantFromDel0]]) == 1.0
    @test !has_start_value(model[:x][1, edgeIndex[supp1FromDel2, xdockFromDel1]])
...
@testset "Extracting paths" begin
    ...
    @constraint(
        model,
        x[1, edgeIndex[supp1FromDel2, xdockFromDel1]] +
        x[1, edgeIndex[xdockFromDel1, plantFromDel0]] +
        x[2, edgeIndex[supp2FromDel1, plantFromDel0]] +
        x[3, edgeIndex[supp1FromDel3, supp1FromDel2]] +
        x[3, edgeIndex[supp1FromDel2, plantFromDel0]] == 5
    )
