...
order1 = OFOND.Order(bunH1, 1, [commodity1, commodity1], 20, bpDict, 10, 5.0)
...
order2 = OFOND.Order(bunH2, 1, [commodity2, commodity2], 30, bpDict, 15, 7.0)
bundle2 = OFOND.Bundle(supplier2, plant, [order2], 2, bunH2, 15, 1)

order3 = OFOND.Order(bunH1, 1, [commodity2, commodity1], 25, bpDict, 10, 6.0)
...
@testset "Constructors" begin
    sol = OFOND.Solution(
        [[1], [2, 3]],
        {1 => [bundle1], 2 => [bundle2, bundle3]},
        sparse([1, 2, 3], [2, 3, 1], [Bin[], Bin[], Bin[]]),
    )
    @test sol.bundlePaths == [[1], [2, 3]]
    @test sol.bundlesOnNode == {1 => [bundle1], 2 => [bundle2, bundle3]}
...
@testset "Update bundle on nodes" begin
    ...
    OFOND.update_bundle_on_nodes!(sol, bundle2, [20, xdockFromDel2, 21]; partial=true)
...
@testset "Add / remove paths" begin
    ...
    @test sol.bundlePaths[1] == Int[]
    @test sol.bundlesOnNode[xdockFromDel2] == [bundle2]
    @test sol.bundlesOnNode[portFromDel1] == [bundle2]
    @test sol.bundlesOnNode[plantFromDel0] == OFOND.Bundle[]
end

dates = [Dates(2020, 1, 1), Dates(2020, 1, 2), Dates(2020, 1, 3), Dates(2020, 1, 4)]
instanceNP = OFOND.Instance(network, TTGraph, TSGraph, bundles, 4, dates)
...
@testset "Check path count" begin
    ...
    @test !check_enough_paths(instance, sol; verbose=false)
    @test_warn "Infeasible solution : 3 bundles and 2 paths" check_enough_paths(
        instance, sol; verbose=true
    )

    sol = OFOND.Solution(TTGraph, TSGraph, bundles)
    @test check_enough_paths(instance, sol; verbose=false)
    @test check_enough_paths(instance, sol; verbose=true)
end

sol = OFOND.Solution(TTGraph, TSGraph, bundles)
@testset "Check supplier / customer" begin
    @test !check_supplier(instance, bundle1, -1; verbose=false)
    @test_warn "Infeasible solution" check_supplier(instance, bundle1, -1; verbose=true)
    @test !check_customer(instance, bundle1, -1; verbose=false)
    @test_warn "Infeasible solution" check_customer(instance, bundle1, -1; verbose=true)

    OFOND.add_path!(sol, bundle1, TTPath)
    @test check_supplier(instance, bundle1, supp1FromDel3; verbose=false)
    @test check_supplier(instance, bundle1, supp1FromDel3; verbose=true)
    @test check_customer(instance, bundle1, plantFromDel0; verbose=false)
    @test check_customer(instance, bundle1, plantFromDel0; verbose=true)
    @test !check_supplier(instance, bundle2, supp1FromDel3; verbose=false)
end
...
@testset "Check quantities" begin
    ...
    @test get_routed_commodities(sol, order1, xdockStep3, portStep3) == OFOND.Commodity[]
    ...
    @test get_routed_commodities(sol, order1, xdockStep3, portStep4) ==
        [commodity1, commodity1, commodity2]
    # check that checker returns false
    @test !check_quantities(
        instance, sol, portFromDel1, plantFromDel0, order1; verbose=false
    )
    @test_warn "Infeasible solution" check_quantities(
    ...
    @test check_quantities(instance, sol, portFromDel1, plantFromDel0, order1; verbose=true)
end
...
@testset "is_feasible" begin
    ...
    for com in [commodity2, commodity1]
        OFOND.add!(sol.bins[supp2Step4, plantStep1][1], com)
    end
...
push!(sol.bins[supp2step3, plantStep1], OFOND.Bin(50))
for com in [commodity2, commodity1]
    OFOND.add!(sol.bins[portStep4, plantStep1][1], com)
end
...
@testset "Cost computation" begin
    ...
    # volume = 30, stockCost = 10.5, distance = 2, unitCost = 10, carbonCost = 1
    @test OFOND.compute_arc_cost(
        TSGraph, bins, supp2step4, plantStep1; current_cost=false
    ) ≈ 61.0
    # volume = 30, stockCost = 10.5, distance = 1, unitCost = 4, carbonCost = 0
    @test OFOND.compute_arc_cost(TSGraph, bins, portStep4, plantStep1; current_cost=false) ≈
        14.5
    # volume = 30, stockCost = 10.5, distance = 1, unitCost = 4, carbonCost = 0, linear
    @test OFOND.compute_arc_cost(
        TSGraph, bins, supp2step4, xdockStep1; current_cost=false
    ) ≈ 13.7
    # test on the whole solution
    # bundle 1 : 0 + 5 + 2.4 + 20 + 5 + 4 = 41.4, bundle 2 : 30 + 14 + 10 = 54.0, bundle 3 : 25 + 12 + 10 = 47.0
    @test OFOND.compute_cost(instance, sol; current_cost=false) ≈ 142.4
end